function [res_admm, out] = admmRedMedianW(A,y,lambda,tol,N,max_iter,median,m,ni,rho,wReg)
...
Aty = A'*y;
% wsq = [wReg(:);wReg(:)].^2;
wsq = [ones(m*ni,1);ones(m*ni,1)].^2;
regProxInv = spdiags(1./(lambda*wsq + rho),0,N,N);
xPrev = x_est;
...
for k = 1:1:max_iter
    ...
    for j = 1:1
        img = reshape(v_est,m,2*ni);
        v_est1 = img((1:m),(1:ni));
        v_est2 = img((1:m),(ni+1:2*ni));
        ...
        v_est = regProxInv*(rho*(x_est + u_est) + lambda*f_v_est);
    end
