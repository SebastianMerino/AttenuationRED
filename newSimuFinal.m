...
resultsDir = 'Q:\smerino\REDjournalResults\phantomSimu\anthony_med7';
...
% freqL = 2.5e6; freqH = 7.5e6; % new bandwidth
wl = 2*c0/(freqL + freqH);
...
inc = (Xq-c1x).^2 + (Zq-c1z).^2 < (rInc-0.15).^2;
back = (Xq-c1x).^2 + (Zq-c1z).^2 > (rInc+0.25).^2;

% [back,inc] = getRegionMasks(xBm,zBm,c1x,c1z,roiL,roiD,roiLz);

% x0mask = c1x - roiL/2; 
% z0mask = c1z - roiLz/2;
groundTruthTargets = [1,1,1,1,1,1,0.5];
