...
% This example simulates focused aquisition with two layers
% Created on May 26th, 2025
% Author: Sebastian Merino

simuNames = {'layered1','layered2'}; 
rng(14)
addpath(genpath(pwd))
...
for iSim = 1:length(simuNames)
    ...
    % Second layer: reference phantom
    layer_std = 0.004;
    layer_rho = rho0*1.1;
    layer_alpha = 0.6;

    % Background
    medium = addRegionSimu([],c0,background_rho,background_std,...
    ...
    switch iSim
        case 1
            maskNodule = rz>2e-2;
        case 2
            maskNodule = rz>-(rx-4e-2).^2/100e-2 + 2e-2;
    end
    medium = addRegionSimu(medium,c0,layer_rho,layer_std,...
    ...
    t3 = nexttile;
    imagesc(100*rx(1,:),100*rz(:,1),medium.alpha_coeff, [0 1])
    xlabel('x [cm]'), ylabel('z [cm]')
    title('Absorption')
