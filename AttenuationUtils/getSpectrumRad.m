function [Sp,Sd,x_ACS,z_ACS,f] = getSpectrumRad(sam1,x,z,fs,blockParams)
%%
dx = (x(2)-x(1)) /100;
dz = (z(2)-z(1)) /100;
...
blocklines = blockParams.blocklines;
...
% Axial samples
wz = round(blocksize*(1-overlap_pc)/dz ); % Between windows
nz = 2*round(blocksize/dz /2); % Window size
...
for iRef = 1:nSamples
    ...
    for jj=1:n
        for ii=1:m
            ...
            Sp_ref(ii,jj,:,iRef) = tempSp;
            Sd_ref(ii,jj,:,iRef) = tempSd;
        end
    end
end
